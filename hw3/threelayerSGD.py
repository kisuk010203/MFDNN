...
torch.manual_seed(0)
X_train = torch.normal(0.0, 1.0, (N,)) # N unit Gaussian RVs
y_train = f_true(X_train) + torch.normal(0, 0.5, X_train.shape)
X_val = torch.normal(0.0, 1.0, (N // 5,))
y_val = f_true(X_val)

train_dataloader = DataLoader(
    TensorDataset(X_train.unsqueeze(1), y_train.unsqueeze(1)), batch_size=B
)
test_dataloader = DataLoader(
    TensorDataset(X_val.unsqueeze(1), y_val.unsqueeze(1)), batch_size=B
)
...
for epoch in range(K):
    for X_batch, y_batch in train_dataloader:
        optimizer.zero_grad()
        loss = nn.MSELoss()(model(X_batch), y_batch) / 2
        loss.backward()
        optimizer.step()

# print("Parameters of MLP model")
# param_num = 0
...
plt.legend()
plt.title("MLP with Gaussian Error in Train Dataset")
plt.savefig("Problem 2_2.pdf")
